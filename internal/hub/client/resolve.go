...
// identity (RepoRef), the host that owns it, and the currently checked-out
// branch. It is the client-side bridge from "the directory the user is in"
// to the path-free identity the hub expects in StartRequest (Phase 3 of
...
// when they exist.
func ResolveRepo(cwd string) (hostID host.HostID, ref host.RepoRef, branch string, err error) {
	...
	return host.HostLocal, ref, branch, nil
}
