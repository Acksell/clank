...
6. **[DONE] Implicit add / clone in `CreateSession`** per §7.5.
   ...
   - Local-kind add (§7.5 step 3) shipped in this step (re-classified after the fact; doc previously said "deferred to step 8").
   ...
   - Integration tests in `internal/host/createsession_test.go`: add-by-Dir success, add-by-Dir mismatch, add-by-Dir not-root, no-hint ErrNotFound, dir-disagrees-with-stored, local-add success, local-add rejects-non-git, clone-with-AllowClone (uses `file://` source URL — full canonicalization support landed in step 8c).
...
8. **[DONE] `StartRequest` finalization + `BackendInvocation` DTO** per §7.3, §7.4. Replace `RepoRemoteURL` (string) with `GitRef` (struct) on the wire. Strip `ProjectDir`/`WorktreeDir`/`ProjectName` from `SessionInfo` and `host.CreateInfo`. Add local-kind registration path (§7.5 step 3). Migrate TUI off deleted fields. `BackendManager.CreateBackend` takes only the DTO.
   - **[DONE] 8e-1**: Migrated consumers (TUI/voice/clankcli) to GitRef-based identity + lazy worktree resolution. SessionInfo path fields still present but unused on read paths.
   - **[DONE] 8e-2a**: Rekeyed agent/model catalog on `(backend, hostname, gitRef.canonical)`. Store v13 migration. Host `ListAgents`/`ListModels` take `GitRef`; host resolves ref→workdir internally.
   - **[DONE] 8e-2b**: Stripped `ProjectDir`/`WorktreeDir`/`ProjectName` from `agent.SessionInfo`; deleted `host.CreateInfo` (§7.1 line 308). `host.Service.CreateSession(req) → (SessionBackend, serverURL, error)` per §7.5. Store v14 migration drops the three columns. `ServerURL` stays on `SessionInfo` (URL, not path) — populated by host on `POST /sessions` response, lost across daemon restart until backend reactivates. Display name everywhere derives from `GitRef.DisplayName()`.
