...
3. **Delete `hostclient.InProcess` + `Client` interface.** Hub depends on `*hostclient.HTTP`. Tests migrate to `httptest.NewServer(hostmux.New(svc))`. Update design doc to reaffirm Decision #3.
4. **Naming + types sweep.** `HostID→Hostname`, drop `RepoID`/`RepoRef`, introduce `GitRef` struct, `Branch→WorktreeBranch`. Pure rename + struct introduction. Unit tests for `GitRef.{Validate, Canonical, Equal}`.
