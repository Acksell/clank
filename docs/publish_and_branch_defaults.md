...
| Credential consent | First `push` per credential triggers a modal asking the user to (a) confirm use of the discovered credential and (b) opt into auto-push / auto-commit. Preferences persist in settings JSON. **Implemented in slimmer form:** the modal only appears on push *failure* (`PushAuthRequiredError`); successful auto-discovery via env / `gh auth token` runs without prompting. See `docs/credential_discovery.md`. The credential-modal retry path bypasses the push confirm modal — a token paste lands directly on a push attempt. |
| Push confirmation | `[p]` opens a confirm modal that owns the in-flight push (spinner, `[r]etry` on non-auth errors, `[esc]` cancel before confirming). Auth errors close the confirm modal and hand off to the credential modal so credentials remain hub-owned and centralised. See `internal/tui/pushconfirm.go` + `internal/tui/inbox_pushconfirm.go`. |
| Push success feedback | Bottom-right toast (`internal/tui/toast.go`) shown for 3s after a successful push. Rendered via lipgloss compositor — does NOT consume a layout row. |
| Row affordance hint | Selected non-default branch rows render `[m]erge  [p]ush` underneath. `[m]erge` is hidden (and the keybind no-ops) when the active host is remote — `git merge` applies to the local working copy and would land on the wrong tree otherwise. See `internal/tui/sidebar_hint.go`. |
| Remote git identity | Remote (Daytona) sandboxes have no `git config user.{name,email}`. The hub seeds them from the user's local `git config --global` once during host provisioning (`internal/hub/hosts_provision.go`) via `POST /identity` on the host mux. Hard-fails when the user has no global identity — no `clank@local` fallback. |
